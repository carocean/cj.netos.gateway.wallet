...
//        byte[] body = new Gson().toJson(rechargeBO).getBytes();
    ...
    @Override
    public Map<String, Object> withdrawOrder(ISecuritySession securitySession, long amount, String payChannelID, String note) throws CircuitException {
        if (amount < 0) {
            throw new CircuitException("500", "金额为负数");
        }
        if (StringUtil.isEmpty(payChannelID)) {
        ...
        withdrawBO.setNote(note);
        withdrawBO.setCurrency("CNY");
        withdrawBO.setPaymentChannelID(payChannelID);
        withdrawBO.setDevice((String) securitySession.property("device"));

        //返回以支付渠道签名的文本
        return callWithdrawOrder(withdrawBO);
    }

    private Map<String, Object> callWithdrawOrder(WithdrawBO bo) throws CircuitException {
        Map<String, Object> mapargs = new HashMap<>();
        mapargs.put("withdrawBill", bo);
        String text = new Gson().toJson(mapargs);
        RequestBody rb = RequestBody.create(text.getBytes());
        ...
        final Request request = new Request.Builder()
                .url(portsUrl)
                .addHeader("Rest-Command", "withdrawOrder")
                .addHeader("app-id", appid)
                .addHeader("app-key", appKey)
                .addHeader("app-nonce", nonce)
                .addHeader("app-sign", sign)
