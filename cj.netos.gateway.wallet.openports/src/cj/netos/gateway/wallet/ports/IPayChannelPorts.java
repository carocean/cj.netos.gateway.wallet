...
import cj.netos.gateway.wallet.bo.PaymentBO;
...
public interface IPayChannelPorts extends IOpenportService {
    ...
    PayChannelResult addPayChannel(ISecuritySession securitySession,
                                   @CjOpenportParameter(usage = "配置信息", name = "payChannel", in = PKeyInRequest.content) PayChannelBO payChannel
    ) throws CircuitException;

    @CjOpenport(usage = "移除渠道")
    PayChannelResult removePayChannel(ISecuritySession securitySession,
                                      @CjOpenportParameter(usage = "支付渠道号", name = "payChannelID") String payChannelID
    ) throws CircuitException;

    @CjOpenport(usage = "分页查询渠道")
    PayChannelResult pagePayChannel(ISecuritySession securitySession,
